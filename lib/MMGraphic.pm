...
method composite (
    ...
    $result_im->Composite( image => $src_graphic, %options );
    ...
    if ( $opacity < 100 ) {
        ...
        $result_im->Composite( image => $result_layer_im );
    }
...
method emboss (
    ...
    if ( $valley_darken ) {
        ...
        $result_im->Composite( image => $valley_darken_im, mask => $valley_mask_im,
...
method apply_mask (
    ...
    $result_im->Composite( image => $combined_mask, compose => 'CopyOpacity'  );
